...
class BaseIO
{
  public:
    ...
    /**
     * The function reads the value from the server. Excluding the command separator.
     *
     ...
     */
    virtual uint32_t write(const uint8_t* _data, uint32_t _size, uint32_t _timeout_ms) = 0;
  ...
  // IEEE Mandated Commands
     ...
     * clearing all the event registers including the Data Questionable Event Register,
     * the Standard Event Status Register, the Standard Operation Status Register and
     * any other registers that are summarized in the status byte.
     *
     * @param _timeout_ms  Timeout in milliseconds. 0 - Timeout disabled
     *
     */
     ...
     * This register monitors I/O errors and synchronization conditions such as operation
     * complete, request control, query error, device dependent error, status execution error,
     * command error and power on. The selected bits are OR’d to become a summary bit (bit 5)
     * in the byte register which can be queried.
     *
    ...
    /**
     * The Operation Complete (OPC) command sets bit 0 in the Standard Event Status
     ...
     * the Standard Event Status Register when all pending operations have finished.
    ...
    /**
     * This reset (RST) command resets most functions to factory-defined conditions.
     * Each command shows *RST value if the setting is affected.
     *
     * @param _timeout_ms  Timeout in milliseconds. 0 - Timeout disabled
     *
     */
     ...
     * The variable <_value> is the decimal sum of the bits that will be enabled.
    ...
    /**
     * The Read Status Byte (STB) query returns the value of the status byte including
     * the master summary status (MSS) bit without erasing its contents.
     * Range: 0–255
     *
     * @param _timeout_ms  Timeout in milliseconds. 0 - Timeout disabled
     *
